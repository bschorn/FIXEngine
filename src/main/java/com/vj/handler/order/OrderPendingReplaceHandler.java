...
public class OrderPendingReplaceHandler extends ExecutionReportHandler {


    public OrderPendingReplaceHandler(EntityTransform entityTransform) {
    ...
    public boolean test(ExecutionReport executionReport, SessionID sessionID) {
        try {
            return executionReport.getExecType().getValue() == ExecType.PENDING_REPLACE;
